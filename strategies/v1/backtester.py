...
class Backtester:
    ...
    def _prepare_data(self, df: pd.DataFrame, feature_names: list) -> pd.DataFrame:
        ...
        if self.config.backtest_days > 0:
            bars_per_day = {'1m': 1440, '15m': 96, '1h': 24, '1d': 1}
    ...
    def _simulate_trading(self, df: pd.DataFrame):
        ...
        for idx, row in df.iterrows():
            ...
            self.equity_curve.append({'timestamp': current_time, 'equity': current_equity, 'position': position})
            ...
            if position == 0:
                ...
                if should_close:
                    ...
                    self.trades.append({
                        ...
                        'pnl_pct': pnl_pct, 'pnl_with_leverage': pnl_with_leverage,
                        'fee': fee, 'net_pnl_pct': net_pnl_pct, 'pnl': pnl,
    ...
    def _calculate_metrics(self) -> dict:
        ...
        return {
            "total_return": float(total_return), "total_return_pct": float(total_return_pct),
            "total_trades": len(trades_df), "winning_trades": len(wins), "losing_trades": len(losses),
            "win_rate": float(win_rate), "profit_factor": float(profit_factor),
            "sharpe_ratio": float(sharpe_ratio), "max_drawdown": float(max_drawdown),
    ...
    def _get_trades_summary(self) -> dict:
        ...
        return {"recent_trades": self.trades[-10:], "total_trades": len(self.trades)}
